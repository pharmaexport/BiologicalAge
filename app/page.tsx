...
function draftsFromAssessment(profile: Profile, questionnaire: Questionnaire): NumericDrafts {
  return {
    ...
    fruitVegServingsPerDay: draftValue(questionnaire.fruitVegServingsPerDay), legumesPerWeek: draftValue(questionnaire.legumesPerWeek), sugaryDrinksPerWeek: draftValue(questionnaire.sugaryDrinksPerWeek),
    nutsSeedsPerWeek: draftValue(questionnaire.nutsSeedsPerWeek), fattyFishPerWeek: draftValue(questionnaire.fattyFishPerWeek), processedMeatPerWeek: draftValue(questionnaire.processedMeatPerWeek),
...
const botanicalFacts = ["Chicorée 3 883 mg", "FOS 3 783 mg", "Fibres solubles 2 000 mg", "Ginseng rouge 100 mg", "Gingembre 60 mg · gingérols 3 mg", "Ginkgo 50 mg", "Scutellaire 33 mg · baïcaline 5 mg", "Quercétine 20 mg", "SOD de melon 1 UI"];
const nutrientFacts = ["Vitamine C 37 mg · 46 %", "Vitamine B3 16 mg · 100 %", "Vitamine D3 25 µg · 500 %", "Vitamine E 2,4 mg · 20 %", "Vitamine A 160 µg · 20 %", "Magnésium 60 mg · 16 %", "Zinc 2 mg · 20 %", "Sélénium 11 µg · 20 %"];
...
export default function HomePage() {
  ...
  const a = (key: string) => antiAgeCopy(language, key);
  ...
  function saveAssessment() { setProfile(effectiveProfile); setQuestionnaire(effectiveQuestionnaire); setDrafts(draftsFromAssessment(effectiveProfile, effectiveQuestionnaire)); window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ profile: effectiveProfile, questionnaire: effectiveQuestionnaire })); setSaved(true); window.setTimeout(() => setSaved(false), 2200); }
  ...
  return <main>
    ...
    {activeView === "assessment" ? <>
      <section className="hero" id="top"><div><p className="eyebrow">{t("eyebrow")}</p><h1>{t("heroTitle")}</h1><p className="heroText">{t("heroText")}</p><div className="actions"><button className="primary" type="button" onClick={saveAssessment}>{saved ? t("saved") : t("save")}</button><button className="secondary" type="button" onClick={resetAssessment}>{t("reset")}</button></div></div><aside className="resultCard" aria-live="polite"><p className="eyebrow">{t("result")}</p><div className="biologicalAge"><span>{t("biologicalAge")}</span><strong>{formatAge(result.biologicalAgeYears, result.biologicalAgeMonths)}</strong></div><div className="metricGrid"><article><span>{t("chronologicalAge")}</span><strong>{result.chronologicalAge} {t("years")}</strong></article><article><span>{t("difference")}</span><strong>{formatDelta(result.deltaMonths)}</strong></article><article><span>{t("score")}</span><strong>{result.score} / 100</strong></article><article><span>{t("confidence")}</span><strong>{confidenceLabel}</strong></article></div></aside></section>
      <section className="assessmentLayout"><form className="assessmentForm" onSubmit={(event) => { event.preventDefault(); saveAssessment(); }}>
        ...
        <fieldset><legend>{t("vitalsTitle")}</legend><div className="formGrid"><NumberField label={t("systolic")} value={drafts.systolic} onChange={(value) => updateDraft("systolic", value)} suffix="mmHg" /><NumberField label={t("diastolic")} value={drafts.diastolic} onChange={(value) => updateDraft("diastolic", value)} suffix="mmHg" /><NumberField label={t("restingHeartRate")} value={drafts.restingHeartRate} onChange={(value) => updateDraft("restingHeartRate", value)} suffix="bpm" /></div></fieldset><button className="primary submitButton" type="submit">{saved ? t("saved") : t("save")}</button>
      </form><aside className="detailsColumn"><section className="detailCard stickyCard"><p className="eyebrow">{t("breakdownTitle")}</p><div className="scoreList">{result.components.map((component) => { const percentage = Math.round((component.score / component.max) * 100); return <article key={component.key}><div><span>{t(componentTranslation[component.key])}</span><strong>{component.score} / {component.max}</strong></div><progress value={percentage} max="100" aria-label={`${t(componentTranslation[component.key])}: ${percentage}%`} /></article>; })}</div></section><section className="detailCard"><h2>{t("favorableTitle")}</h2><ul>{result.favorable.length ? result.favorable.map((factor) => <li key={factor}>{t(factorTranslation[factor])}</li>) : <li>{t("noneReported")}</li>}</ul></section><section className="detailCard warningCard"><h2>{t("watchTitle")}</h2><ul>{result.unfavorable.length ? result.unfavorable.map((factor) => <li key={factor}>{t(factorTranslation[factor])}</li>) : <li>{t("noneReported")}</li>}</ul></section><section className="detailCard methodCard"><h2>{t("methodTitle")}</h2><p>{t("methodText")}</p></section></aside></section>
    </> : <section className={styles.panel} id="top">
      <header className={styles.masthead}><div className={styles.mastheadContent}><div className={styles.logo}><span className={styles.logoMark} aria-hidden="true" /><span>Arko<small>PHARMA</small></span></div><p className={styles.kicker}>{a("tab")}</p><h2>{a("title")}</h2><p>{a("intro")}</p></div></header>
      <div className={styles.grid}>{priorities.map((priority, index) => <article className={styles.card} key={priority.key}><div className={styles.rank}>{index + 1}</div><div className={styles.gainLabel}>{a("gain")}</div><div className={styles.gain}><strong>{priority.estimateMinMonths}–{priority.estimateMaxMonths}</strong><span>{a("months")}</span></div><h3>{t(priority.titleKey)}</h3><div className={styles.dataBlock}><b>{a("objective")}</b><p>{a(priority.targetKey)}</p></div><div className={styles.dataBlock}><b>{a("evidence")}</b><p>{a(priority.evidenceKey)}</p></div><span className={styles.confidence}>{a("confidence")} · {a(priority.confidence)}</span></article>)}</div>
      <section className={styles.product}><div className={styles.productIntro}><div className={styles.logo}><span className={styles.logoMark} aria-hidden="true" /><span>Arko<small>PHARMA</small></span></div><p className={styles.kicker}>{a("productEyebrow")}</p><h2>{a("productTitle")}</h2><p>{a("productIntro")}</p><span className={styles.doseBadge}>{a("dose")}</span></div><div className={styles.facts}><article className={styles.factGroup}><h3>{a("botanicals")}</h3><div className={styles.factGrid}>{botanicalFacts.map((fact) => { const [name, ...rest] = fact.split(" "); return <div className={styles.fact} key={fact}><span>{name}</span><strong>{rest.join(" ")}</strong></div>; })}</div></article><article className={styles.factGroup}><h3>{a("nutrients")}</h3><div className={styles.factGrid}>{nutrientFacts.map((fact) => { const [name, ...rest] = fact.split(" "); return <div className={styles.fact} key={fact}><span>{name}</span><strong>{rest.join(" ")}</strong></div>; })}</div></article><article className={`${styles.factGroup} ${styles.warning}`}><h3>{a("caution")}</h3><p>{a("cautionText")}</p></article><p className={styles.disclaimer}>{a("disclaimer")}</p></div></section>
    </section>}
  </main>;
}
